	...
	private final class Node {
		...
		public Node insert(T newData, boolean[] rta) {
			...
			if (this.data.compareTo(newData) == 0) {
				...
				//this.data > newData, INSERTO detras de mi!
				rta[0] = true;
				Node aux = new Node(newData, this, this.prev);
				this.prev = aux;
				return aux;
			} else if (this.next == null) {
				//this.data < newData y estoy en el final de la lista, inserto adelante
				rta[0] = true;
				Node aux = new Node(newData, null, this);
				this.next = aux;
				return this;
			} else {
		...
		public Node remove(T removeData, boolean[] rta){
			if(data.compareTo(removeData) > 0){
				...
				if (next != null) {
					next.prev = this.prev; // Actualizamos la referencia prev del nodo siguiente
				}
				if (prev != null) {
					prev.next = this.next; // Actualizamos la referencia next del nodo anterior
				}
				return next;
			} else if( next == null ){
