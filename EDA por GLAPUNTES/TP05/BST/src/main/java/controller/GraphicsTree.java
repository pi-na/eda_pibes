...
import FiboTree.NodeFiboInterface;
...
public class GraphicsTree<T extends Comparable<? super T>> extends Canvas  {
	...
	private NodeFiboInterface root;
	...
	public GraphicsTree(FiboTree myTree) {
	...
	private void drawLines(GraphicsContext gc, NodeFiboInterface treeNode, double xMin, double xMax, double yMin, double yMax) {
		Point2D p1, p2;
		Line newLine;
	...
	private void drawCircles(GraphicsContext gc, NodeFiboInterface treeNode, double xMin, double xMax, double yMin, double yMax) {

		// Create a new point
		Point2D point = new Point2D((xMin + xMax) / 2, yMin + yMax / 2);

		// Draw the circle
		Circle x = new Circle(treeNode.getData(), point);
		x.draw(gc);
