...
public class IndexWithDuplicatesParam<T extends Comparable<? super T>> implements IndexServiceParam<T> {
    ...
    public int getClosestPosition(T[] array, T key, int leftIndex, int rightIndex){
//        System.out.println("left/array[left]" + left + "/" + array[left] + "    | right/array[right]:" + right + "/" + array[right]);
        if(leftIndex == rightIndex) {
            return leftIndex;
        }
        if(leftIndex > rightIndex) {
//            System.out.println("left" + left + ">" + right + " retornando - 1");
            return -1;
        }
