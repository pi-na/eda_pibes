...
public class IndexWithDuplicates<T extends Comparable<? super T>> implements IndexService<T>{
    ...
    public boolean search(T key) {
        ...
        int closestPosition = getClosestPosition(key, 0, dim-1);
        if(closestPosition<0 || closestPosition>=dim)
            return false;
        return ((T)elements[closestPosition]).equals(key);
    }

    // Devuelve el índice del elemento más cercano a key
    public int getClosestPosition(T key, int izq, int der) {
        if(izq >= der) {
        ...
        if(((T)elements[pivot]).compareTo(key)>0)
            return getClosestPosition(key, izq, pivot-1);
        if(((T)elements[pivot]).compareTo(key)<0)
            return getClosestPosition(key, pivot+1, der);

        return pivot;
    }

    public int firstOccurrence(T key) {
        int index = getClosestPosition(key, 0, dim-1);
