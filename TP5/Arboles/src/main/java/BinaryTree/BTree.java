...
public class BTree<T extends Comparable<T>> implements ITree<T> {
    ...
    private T remove(T value, Node<T> node) {
        ...
        if (node.numberOfChildren() == 0) {
            ...
            Node<T> greatest = node.getChild(index + 1);
            System.out.println(Arrays.toString(greatest.keys));
            Node<T> lesser = this.getLesserNode(greatest);
            System.out.println(Arrays.toString(lesser.keys));
            T replaceValue = this.removeLesserValue(lesser);
            //T replaceValue = this.removeGreatestValue(greatest);
            node.addKey(replaceValue);
            if (lesser.parent != null && lesser.numberOfKeys() < minKeySize) {
                this.combined(lesser);
            }
            if (lesser.numberOfChildren() > maxChildrenSize) {
                this.split(lesser);
            }
        }
    ...
    private T removeLesserValue(Node<T> node){
        T value = null;
        if (node.numberOfKeys() > 0) {
