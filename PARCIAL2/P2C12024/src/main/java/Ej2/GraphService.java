...
public interface GraphService<V,E> {
	...
	// Only throws a RuntimeException if the graph is directed, 
	// multi o  default graph (self-loop).
	//  Otherwise, if the graph is simple, then returns a number:
	// -1 if the graph is not regular.
	// N if the graphs is a N-regular one.
